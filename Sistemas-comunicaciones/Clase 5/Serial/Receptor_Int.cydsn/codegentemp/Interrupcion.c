...
void Interrupcion_SetVector(cyisraddress address)
{
    ...
    ramVectorTable[CYINT_IRQ_BASE + (uint32)Interrupcion__INTC_NUMBER] = address;
}
...
cyisraddress Interrupcion_GetVector(void)
{
    ...
    return ramVectorTable[CYINT_IRQ_BASE + (uint32)Interrupcion__INTC_NUMBER];
}
...
{
    *Interrupcion_INTC_PRIOR = priority << 5;
}
...
uint8 Interrupcion_GetPriority(void)
{
    ...
    priority = *Interrupcion_INTC_PRIOR >> 5;
