library(tidyverse)
library(datos)
#Caso I: Una variable categórica y una continua
...
ggplot(data = aviones, mapping = aes(x = asientos, y = ..density..)) +
  geom_freqpoly(mapping = aes(colour = tipo), binwidth = 50)
...
ggplot(data = aviones) +
  geom_boxplot(mapping = aes(x = reorder(x = tipo, asientos, FUN = median),
  y = asientos))
...
aviones %>%
  count(tipo_motor, tipo)

aviones %>%
  count(tipo_motor, tipo) %>%
  ggplot(mapping = aes(x = tipo_motor, y = tipo)) +
  geom_tile(mapping = aes(fill = n))

#Abra el conjunto de datos atmostera
atm <- datos::atmosfera
#y grafique
ggplot(data = atm) +
