...
***************************************/
static void ADC_SAR_CalcGain(uint8 resolution);
...
volatile int16 ADC_SAR_countsPerVolt;     /* Obsolete Gain compensation */
volatile int32 ADC_SAR_countsPer10Volt;   /* Gain compensation */
...
*  ADC_SAR_Start() sets the initVar variable, calls the
*  ADC_SAR_Init() function, and then calls the
...
void ADC_SAR_Enable(void)
{
    ...
    tmpReg = ADC_SAR_SAR_CSR3_REG;
    tmpReg |= ADC_SAR_SAR_EN_BUF_VCM_EN;
    /* PD_BUF_VREF is OFF in External reference or Vdda reference mode */
    #if((ADC_SAR_DEFAULT_REFERENCE == ADC_SAR__EXT_REF) || \
        (ADC_SAR_DEFAULT_RANGE == ADC_SAR__VNEG_VDDA_DIFF))
        tmpReg &= (uint8)~ADC_SAR_SAR_EN_BUF_VREF_EN;
    #else /* In INTREF or INTREF Bypassed this buffer is ON */
        tmpReg |= ADC_SAR_SAR_EN_BUF_VREF_EN;
    #endif /* ADC_SAR_DEFAULT_REFERENCE == ADC_SAR__EXT_REF */
    ADC_SAR_SAR_CSR3_REG = tmpReg;

    /* Set reference for ADC */
    #if(ADC_SAR_DEFAULT_RANGE == ADC_SAR__VNEG_VDDA_DIFF)
        #if(ADC_SAR_DEFAULT_REFERENCE == ADC_SAR__EXT_REF)
            ADC_SAR_SAR_CSR6_REG = ADC_SAR_INT_BYPASS_EXT_VREF; /* S2 */
        #else /* Internal Vdda reference or obsolete bypass mode */
            ADC_SAR_SAR_CSR6_REG = ADC_SAR_VDDA_VREF;           /* S7 */
        #endif /* ADC_SAR_DEFAULT_REFERENCE == ADC_SAR__EXT_REF */
    #else  /* Reference goes through internal buffer */
        #if(ADC_SAR_DEFAULT_REFERENCE == ADC_SAR__INT_REF_NOT_BYPASSED)
            ADC_SAR_SAR_CSR6_REG = ADC_SAR_INT_VREF;            /* S3 + S4 */
        #else /* INTREF Bypassed of External */
            ADC_SAR_SAR_CSR6_REG = ADC_SAR_INT_BYPASS_EXT_VREF; /* S2 */
        #endif /* ADC_SAR_DEFAULT_REFERENCE == ADC_SAR__INT_REF_NOT_BYPASSED */
    #endif /* VNEG_VDDA_DIFF */

    /* Low non-overlap delay for sampling clock signals (for 1MSPS) */
    #if(ADC_SAR_HIGH_POWER_PULSE == 0u) /* MinPulseWidth <= 50 ns */
        ADC_SAR_SAR_CSR5_REG &= (uint8)~ADC_SAR_SAR_DLY_INC;
    #else /* Set High non-overlap delay for sampling clock signals (for <500KSPS)*/
        ADC_SAR_SAR_CSR5_REG |= ADC_SAR_SAR_DLY_INC;
    #endif /* ADC_SAR_HIGH_POWER_PULSE == 0u */
    ...
    */
    #if(ADC_SAR_HIGH_POWER_PULSE == 0u)    /* MinPulseWidth <= 50 ns */
        ADC_SAR_SAR_CSR5_REG |= ADC_SAR_SAR_SEL_CSEL_DFT_CHAR;
    #else /* for <500ksps */
        ADC_SAR_SAR_CSR5_REG &= (uint8)~ADC_SAR_SAR_SEL_CSEL_DFT_CHAR;
    #endif /* ADC_SAR_HIGH_POWER_PULSE == 0u */

    /* Set default power and other configurations for control register 0 in multiple lines */
    ADC_SAR_SAR_CSR0_REG = (uint8)((uint8)ADC_SAR_DEFAULT_POWER << ADC_SAR_SAR_POWER_SHIFT)
    /* SAR_HIZ_CLEAR:   Should not be used for LP */
    #if ((CY_PSOC5LP) || (ADC_SAR_DEFAULT_REFERENCE != ADC_SAR__EXT_REF))
        | ADC_SAR_SAR_HIZ_CLEAR
    #endif /* SAR_HIZ_CLEAR */
    /*Set Convertion mode */
    #if(ADC_SAR_DEFAULT_CONV_MODE != ADC_SAR__FREE_RUNNING)      /* If triggered mode */
        | ADC_SAR_SAR_MX_SOF_UDB           /* source: UDB */
    ...
    ADC_SAR_SAR_TR0_REG = ADC_SAR_SAR_CAP_TRIM_2;
...
void ADC_SAR_Stop(void)
{
    ...
    ADC_SAR_SAR_CSR0_REG &= (uint8)~ADC_SAR_SAR_SOF_START_CONV;
    /* Disable the SAR ADC block in Active Power Mode */
    ADC_SAR_PWRMGR_SAR_REG &= (uint8)~ADC_SAR_ACT_PWR_SAR_EN;
    /* Disable the SAR ADC in Standby Power Mode */
    ADC_SAR_STBY_PWRMGR_SAR_REG &= (uint8)~ADC_SAR_STBY_PWR_SAR_EN;

    /* This is only valid if there is an internal clock */
    #if(ADC_SAR_DEFAULT_INTERNAL_CLK)
        ADC_SAR_PWRMGR_CLK_REG &= (uint8)~ADC_SAR_ACT_PWR_CLK_EN;
        ADC_SAR_STBY_PWRMGR_CLK_REG &= (uint8)~ADC_SAR_STBY_PWR_CLK_EN;
    #endif /* End ADC_SAR_DEFAULT_INTERNAL_CLK */
...
void ADC_SAR_SetPower(uint8 power)
{
    ...
    /* Set Power parameter  */
    tmpReg = ADC_SAR_SAR_CSR0_REG & (uint8)~ADC_SAR_SAR_POWER_MASK;
    tmpReg |= (uint8)(power << ADC_SAR_SAR_POWER_SHIFT);
...
void ADC_SAR_SetResolution(uint8 resolution)
{
    ...
    switch (resolution)
    {
        case (uint8)ADC_SAR__BITS_12:
            tmpReg = ADC_SAR_SAR_RESOLUTION_12BIT | ADC_SAR_SAR_SAMPLE_WIDTH;
            break;
        case (uint8)ADC_SAR__BITS_10:
            tmpReg = ADC_SAR_SAR_RESOLUTION_10BIT | ADC_SAR_SAR_SAMPLE_WIDTH;
            break;
        case (uint8)ADC_SAR__BITS_8:
            tmpReg = ADC_SAR_SAR_RESOLUTION_8BIT | ADC_SAR_SAR_SAMPLE_WIDTH;
            break;
        default:
            tmpReg = ADC_SAR_SAR_RESOLUTION_12BIT | ADC_SAR_SAR_SAMPLE_WIDTH;
    ...
    *  ADC_SAR_StartConvert(). This function is not available when the
    ...
    *******************************************************************************/
    ...
    {
        #if(ADC_SAR_DEFAULT_CONV_MODE != ADC_SAR__FREE_RUNNING)  /* If software triggered mode */
            ADC_SAR_SAR_CSR0_REG &= (uint8)~ADC_SAR_SAR_MX_SOF_UDB;   /* source: SOF bit */
...
*   ADC_SAR_RETURN_STATUS: Immediately returns the status. If the
...
*   ADC_SAR_WAIT_FOR_RESULT: Does not return a result until the ADC
...
*
* Global Variables:
*  ADC_SAR_shift - used to convert the ADC counts to the 2s
*  compliment form.
*
* Side Effects:
...
{
    return( (int8)ADC_SAR_SAR_WRK0_REG - (int8)ADC_SAR_shift);
}
...
int16 ADC_SAR_GetResult16( void )
{
    ...
    res = CY_GET_REG16(ADC_SAR_SAR_WRK_PTR);

    return( (int16)res - ADC_SAR_shift );
}
...
*
* Summary:
*  Sets the ADC offset, which is used by ADC_SAR_CountsTo_Volts(),
...
*  ADC_SAR_countsPer10Volt variable initialized. This variable is used
...
static void ADC_SAR_CalcGain( uint8 resolution )
{
    int32 counts;
         ...
         (ADC_SAR_DEFAULT_RANGE == ADC_SAR__VSSA_TO_VDDA) || \
         (ADC_SAR_DEFAULT_RANGE == ADC_SAR__VSSA_TO_VDAC)) )
        uint16 diff_zero;
    ...
    switch (resolution)
    {
        case (uint8)ADC_SAR__BITS_12:
            counts = (int32)ADC_SAR_SAR_WRK_MAX_12BIT;
                 ...
                 (ADC_SAR_DEFAULT_RANGE == ADC_SAR__VSSA_TO_VDDA) || \
                 (ADC_SAR_DEFAULT_RANGE == ADC_SAR__VSSA_TO_VDAC)) )
                diff_zero = ADC_SAR_SAR_DIFF_SHIFT;
            #endif /* End ADC_SAR_DEFAULT_RANGE */
            break;
        case (uint8)ADC_SAR__BITS_10:
            counts = (int32)ADC_SAR_SAR_WRK_MAX_10BIT;
            #if(!((ADC_SAR_DEFAULT_RANGE == ADC_SAR__VSS_TO_VREF) || \
                 (ADC_SAR_DEFAULT_RANGE == ADC_SAR__VSSA_TO_VDDA) || \
                 (ADC_SAR_DEFAULT_RANGE == ADC_SAR__VSSA_TO_VDAC)) )
                diff_zero = ADC_SAR_SAR_DIFF_SHIFT >> 2u;
            #endif /* End ADC_SAR_DEFAULT_RANGE */
            break;
        case (uint8)ADC_SAR__BITS_8:
            counts = (int32)ADC_SAR_SAR_WRK_MAX_8BIT;
            #if(!((ADC_SAR_DEFAULT_RANGE == ADC_SAR__VSS_TO_VREF) || \
                 (ADC_SAR_DEFAULT_RANGE == ADC_SAR__VSSA_TO_VDDA) || \
                 (ADC_SAR_DEFAULT_RANGE == ADC_SAR__VSSA_TO_VDAC)) )
                diff_zero = ADC_SAR_SAR_DIFF_SHIFT >> 4u;
            #endif /* End ADC_SAR_DEFAULT_RANGE */
            break;
    ...
    ADC_SAR_countsPerVolt = 0; /* Clear obsolete variable */
    /* Calculate gain in counts per 10 volts with rounding */
    ADC_SAR_countsPer10Volt = (((counts * ADC_SAR_10MV_COUNTS) +
                        ADC_SAR_DEFAULT_REF_VOLTAGE_MV) / (ADC_SAR_DEFAULT_REF_VOLTAGE_MV * 2));
...
*
* Global Variables:
*  ADC_SAR_countsPer10Volt variable modified. This variable is used
*  for gain calibration purpose.
*
*******************************************************************************/
void ADC_SAR_SetGain(int16 adcGain)
{
    ADC_SAR_countsPer10Volt = (int32)adcGain * 10;
}


/*******************************************************************************
* Function Name: ADC_SAR_SetScaledGain
********************************************************************************
*
* Summary:
...
{
    ADC_SAR_countsPer10Volt = adcGain;
}
...
*  ADC_SAR_countsPer10Volt variable used.
*
*******************************************************************************/
int16 ADC_SAR_CountsTo_mVolts(int16 adcCounts)
{
    ...
    if(ADC_SAR_countsPerVolt != 0)
    {   /* Support obsolete method */
        countsPer10Volt = (int32)ADC_SAR_countsPerVolt * 10;
    }
    else
    {
        countsPer10Volt = ADC_SAR_countsPer10Volt;
    }
...
int32 ADC_SAR_CountsTo_uVolts(int16 adcCounts)
{
    ...
    */
    uVolts = (( (int64)adcCounts * ADC_SAR_10UV_COUNTS ) / countsPer10Volt);
...
float32 ADC_SAR_CountsTo_Volts(int16 adcCounts)
{
    ...
    volts = ((float32)adcCounts * ADC_SAR_10V_COUNTS) / (float32)countsPer10Volt;
